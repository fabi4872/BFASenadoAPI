      ...
      ""type"": ""function""
        ...
        public async Task<IActionResult> GetHashes()
        {
            try
            {
                ...
                return Ok(new { success = true, data = hashStrings });
            }
            catch (Exception ex)
            {
                return BadRequest(new { success = false, message = $"Error al interactuar con el contrato: {ex.Message}" });
            }
        }

        // Guardar un nuevo hash
        [HttpPost("GuardarHash")]
        public async Task<IActionResult> StoreHash([FromBody] HashInput input)
        {
            try
            {
                if (string.IsNullOrEmpty(input.Hash))
                {
                    return BadRequest(new { success = false, message = "Hash no puede ser nulo o vacío." });
                }
                ...
                if (true)
                {
                    ...
                    if (transaccion != null)
                    {
                        ...
                        return Ok(new { success = true, transactionHash });
                    }
                }

                return BadRequest(new { success = false, message = $"Error al almacenar el hash." });
            }
            catch (Exception ex)
            {
                return BadRequest(new { success = false, message = $"Error al almacenar el hash: {ex.Message}" });
            }
        }

        // Consultar la información de un hash
        [HttpGet("Hash")]
        public async Task<IActionResult> GetHashData([FromQuery] string hash)
        {
            try
            {
                if (string.IsNullOrEmpty(hash))
                    return BadRequest(new { success = false, message = "El hash proporcionado no es válido." });
            ...
            {
                return BadRequest(new { success = false, message = $"Error al consultar el hash: {ex.Message}" });
            }
        }
